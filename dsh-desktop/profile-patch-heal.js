...
/**
 * 备份 + 原子写回：把坏 bundle 从 dsh.profile.bundles 与其 dependencies 条目
 * 一并移除。先 copyFileSync 备份（.bak-<ts>，settings .broken-<ts> 先例），
 * 无实际变化时零写入。返回实际移除的名单。
...
function removeBundlesFromProfile(profileDir, names, fs = require('node:fs')) {
  ...
  const deps = Object.fromEntries(
    Object.entries(manifest.dependencies || {}).filter(([n]) => !wanted.has(n))
  );
  if (bundles.length === before.length && Object.keys(deps).length === Object.keys(manifest.dependencies || {}).length) {
    return [];
  }
  fs.copyFileSync(pkgFile, pkgFile + '.bak-' + Date.now());
  manifest.dsh = manifest.dsh || {};
  manifest.dsh.profile = manifest.dsh.profile || {};
  manifest.dsh.profile.bundles = bundles;
  manifest.dependencies = deps;
  fs.writeFileSync(pkgFile, JSON.stringify(manifest, null, 2) + '\n');
