...
class PluginStateStore {
   ...
   */
  constructor({ file, log = () => {}, gate } = {}) {
    this.file = file;
    this.log = log;
    this.gate = gate || sharedWriteGate(path.join(path.dirname(file), '.dsh-locks'));
    this.data = this.load();
    // 自上次落盘以来被删除的键（跨进程合并时用于不复活已清除的决策）。
  ...
  load() {
    ...
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)
      ...
      this.log('插件状态文件损坏（已备份到 ' + backup + '），重建为空状态；卸载/隔离决策以 cordis.patch.yml 行为准');
  ...
  /**
   * 落盘（原子写 + 写锁）。锁内重读磁盘做「按 key 合并」：新增/修改的键写入、
   * 本实例删除过的键移除、其余键保留——避免「壳层与 CLI 各自持有内存快照、
   * 后写者整份覆盖前写者决策」的丢更新。返回是否成功（false=失败，仅告警）。
   */
  save() {
      ...
      try {
        ...
        if (raw !== null) {
          try {
            ...
            if (parsed && typeof parsed === 'object' && !Array.isArray(parsed) && parsed.v === STATE_VERSION) {
              disk = {
                v: STATE_VERSION,
                uninstalled: sanitizeMap(parsed.uninstalled),
                quarantine: sanitizeMap(parsed.quarantine),
        ...
        const merged = this.emptyState();
        merged.uninstalled = { ...disk.uninstalled, ...this.data.uninstalled };
        merged.quarantine = { ...disk.quarantine, ...this.data.quarantine };
  ...
  markUninstalled(id, name, source = 'ui') {
    if (!isLoaderId(id)) return Promise.resolve(false);
