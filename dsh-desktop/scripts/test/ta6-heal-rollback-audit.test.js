...
//
// 对 32 个 file transform 逐个回答「如何撤销」：
...
//
// 审计约束（守卫价值）：
//   1. 分类必须覆盖全部 32 个 file transform（无「无法回滚」盲区）；
...
test('审计 1：分类覆盖全部 32 个 file transform（无回滚盲区）', () => {
  assert.equal(fileSpecs.length, 32);
  const report = [];
...
test('审计 3：root 应用器只碰 node_modules（npm ci 整体可恢复）', () => {
  assert.equal(rootSpecs.length, 14);
