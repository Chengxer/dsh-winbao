...
//
// 对 28 个 file transform 逐个回答「如何撤销」：
...
//
// 审计约束（守卫价值）：
//   1. 分类必须覆盖全部 28 个 file transform（无「无法回滚」盲区）；
//   2. 每个带 marker 的 transform，marker 必须能定位回滚点（marker 出现在
//      其 changed 产物中——用 pristine 实跑验证）；
//   3. root 应用器（8 个）只碰 node_modules 内文件 → npm ci 可整体恢复。
// ---------------------------------------------------------------------------
...
test('审计 1：分类覆盖全部 28 个 file transform（无回滚盲区）', () => {
...
test('审计 3：root 应用器只碰 node_modules（npm ci 整体可恢复）', () => {
  assert.equal(rootSpecs.length, 10);
