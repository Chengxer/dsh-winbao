...
// ---------------------------------------------------------------------------
// TA6 元测试 6：47 补丁 × 双版本形态判定矩阵（基线快照，最有长期价值）。
//
...
test('47 补丁 × rc.2 / rc.1 双形态判定矩阵与基线快照一致（锚点漂移哨兵）', { skip: !formRoot('rc.2') ? 'pristine rc.2 stage 树不可用（.tmp-rc2-stage 缺失）' : false }, () => {
...
test('基线快照自身完整性：两形态 × 47 id 全覆盖', () => {
  const ids = new Set(PATCH_SPECS.map((s) => s.id));
  assert.equal(ids.size, 47);
  for (const form of Object.keys(BASELINE)) {
    assert.equal(Object.keys(BASELINE[form]).length, 47, `${form} 基线应覆盖 47 项`);
