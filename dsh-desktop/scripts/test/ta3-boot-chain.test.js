...
// ~/.dsh 与在用实例），跑生产 applyAll（44 补丁注册表全量）+ 只读预检 +
// 关键服务修复探测，断言：
//   1. 一遍：44 补丁全部执行、changed > 0、零 errors；
...
test('boot 链一条龙：applyAll(44) → composition-integrity → preflight → 二遍幂等', { skip: !hasPristine() && '缺 .tmp-rc2-stage pristine 源' }, async (t) => {
  const { appDir, home, ctx } = await buildTempRoots(t);

  // ---- 1. 一遍 applyAll：44 补丁全执行、有落盘、零 errors ----
  const r1 = applyAll(ctx);
  assert.equal(r1.total, 44, `注册表应有 44 个补丁（实际 ${r1.total}）`);
  assert.equal(PATCH_SPECS.length, 44, 'PATCH_SPECS 与编排 total 一致');
