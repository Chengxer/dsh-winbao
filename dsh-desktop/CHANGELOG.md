...
- **识图插件多模态体感：直接发图、后台自动识别（dsh-vision）**：输入框工具行新增「🖼 添加图片」按钮（也可粘贴/拖放图片），图片按官方附件链路进入草稿并随消息发送；宿主半边在 `agent/pre-step` 边界拦截消息中的图片块，调 VLM 识别后替换为「[图片] 识别结果」文本（多图自动编号 1/N；用户消息文本自动作为识别问题；识别失败/超限/附件异常一律降级为说明文本，绝不阻断对话），纯文本模型全程只见文本——发图体验与多模态模型一致，识图完全隐藏在后台。识别结果按附件 id 缓存（同图跨轮不重复请求）；`view_image` 工具与自动识别共用同一模型/备用链/超时/降档配置
