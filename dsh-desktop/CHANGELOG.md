...
### 修复
- **识图插件发图被拒「图片发送失败（该模型不支持图片输入）」**：宿主 apiproxy 在 prompt 入口就校验模型 `inputModalities` 是否含 `image`，而 DeepSeek 适配器声明 text-only——发送即被拒，dsh-vision 的 `llm/stream` 识别拦截（更下游）根本执行不到。现 dsh-vision 在适配器层 wrap 每个 LLM adapter 的 `resolveModel`/`listModels`（`Object.create` 原型继承 + 幂等标记，与 dsh-third-party-thinking 同款机制，多插件 wrap 链式兼容；监听 `llm/adapters-updated` 覆盖热装配）：文本型模型声明 `["text","image"]` 放行 prompt 检查，图片照常进入会话、由 `llm/stream` 拦截转为识别文本后送达模型。同时 wrap 记录各模型的**原生**多模态能力，拦截器据此透传——原生多模态模型（如 pi-ai）收到的仍是原图，不会被降级为识别文本。新增 4 个 `supportsImages` 单测（原生透传 / 文本型识别 / 判定异常保守识别 / 缺省识别）+ 6 个 wrap 单测（text-only 放行与缓存、原生多模态透传、未声明保守、幂等、第三方 wrap 链式兼容、异常不写缓存），dsh-vision 单测 33/33
